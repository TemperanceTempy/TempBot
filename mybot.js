...
client.on("ready", () => {
  ...
  if (!table['count(*)']) {
    ...
    sql.prepare("CREATE TABLE scores (id TEXT PRIMARY KEY, user TEXT, guild TEXT, points INTEGER, uses INTEGER);").run();
  ...
  client.setScore = sql.prepare("INSERT OR REPLACE INTO scores (id, user, guild, points, uses) VALUES (@id, @user, @guild, @points, @uses);");
});
...
client.on("message", message => {
  ...
  if(command==="uses"){
    return message.reply(`You've used this bots commands ${score.uses} times!`)
  }
  if(command === "points") {
    score.uses++;
